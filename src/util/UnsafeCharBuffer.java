...
public class UnsafeCharBuffer {
    ...
    static
    {
        try
        {
            hbOffset = UnsafeAccess.unsafe.objectFieldOffset(CharBuffer.class.getDeclaredField("hb"));
            offsetOffset = UnsafeAccess.unsafe.objectFieldOffset(CharBuffer.class.getDeclaredField("offset"));
            capacityOffset = UnsafeAccess.unsafe.objectFieldOffset(Buffer.class.getDeclaredField("capacity"));
            markOffset = UnsafeAccess.unsafe.objectFieldOffset(Buffer.class.getDeclaredField("mark"));
        }
        catch (Exception e)
        {
    ...
    public static void wrap(CharBuffer buffy,char[] chars, int offset, int length){
        ...
        UnsafeAccess.unsafe.putInt(buffy,offsetOffset,0); // see CharBuffer.wrap doc
